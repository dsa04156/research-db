...
1. If `skill-observations/log.md` or `cross-cutting-principles.md` don't
   ...
   If the active task defines a clean-worktree or repository pre-sync
   gate, inspect and record that initial state before creating observer
   files. Observer instrumentation must not change the condition that the
   task's preflight is about to measure.
